...
Projeto desenvolvido em C# (.NET 8) como parte do Trabalho III da disciplina de Sistemas de Bancos de Dados.

## ⚙️ Funcionalidades Implementadas

O sistema atende a todos os requisitos propostos:
- [x] Leitura de escalonamentos a partir de arquivo de texto.
...
- [x] Criação automática de arquivos de log individuais para cada objeto de dado detalhando o ID do escalonamento, a operação e o momento exato em que ocorreu.

## 📂 Formato de Entrada

**Atenção:** O parser do projeto está configurado para ler os dados a partir do arquivo `infinal.txt` localizado na raiz do projeto. 

Se você quiser testar os outros arquivos de entrada disponíveis no repositório, basta abrir o código-fonte do arquivo parser.cs e alterar o valor da variável in_path para o nome do arquivo desejado.
...
O projeto foi desenvolvido em C# utilizando o SDK do .NET 8. Para rodar o projeto localmente no seu terminal (Linux, Windows ou macOS):
...
Após a execução, os logs do console mostrarão o processamento (Objetos, Transações, Timestamps e Escalonamentos) e o arquivo `out.txt`, bem como os logs individuais dos objetos de dados, serão gerados automaticamente na pasta do projeto.
